...
<level_info>
The following steps are in the same execution level (no dependencies between them)
and could potentially be combined:
{level_steps_text}
</level_info>

<instructions>
You are evaluating which steps in the same execution level can be safely merged
into a single step. Steps should only be merged if:

1. **Semantic relatedness**: The steps work on closely related functionality or data
2. **Combined complexity**: Merging them does not create an overly complex single step
3. **Independence**: The steps don't produce separate outputs that other steps depend on individually

For each proposed merge, explain why the steps are related and why merging is safe.

Return ONLY a JSON object:
{{"merges": [[step_id_a, step_id_b], ...], "reasoning": "brief explanation"}}

If no steps should be merged, return:
{{"merges": [], "reasoning": "brief explanation why no merges are appropriate"}}

Step IDs are 1-based integers matching the step numbers above.
</instructions>
"""


def merge_steps_with_llm(goal: str, steps: list[dict]) -> list[dict]:
    """Use LLM to decide which steps to merge based on semantic relatedness.

    Computes execution levels, submits levels with 2+ steps to the LLM
    for merge evaluation. Falls back to merge_trivial_steps() on failure.
    """
    if len(steps) < 2:
        return steps
    ...
    # Check if any level has 2+ steps (otherwise nothing to merge)
    multi_step_levels = [lvl for lvl in levels if len(lvl) >= 2]
    if not multi_step_levels:
        return steps
    ...
    for i, step in enumerate(steps):
        ...
        deps_str = f" (depends on: {deps})" if deps else ""
        steps_desc.append(
            f"Step {i + 1}: {step.get('title', 'Untitled')}{deps_str}\n"
            f"  {step.get('description', '')}"
        )
    steps_text = "\n\n".join(steps_desc)

    # Format level steps for evaluation
    level_parts = []
    for lvl in multi_step_levels:
        step_ids = ", ".join(str(sid) for sid in lvl)
        titles = ", ".join(
            f"Step {sid} ({steps[sid - 1].get('title', 'Untitled')})"
            for sid in lvl
        )
        level_parts.append(f"Level [{step_ids}]: {titles}")
    level_steps_text = "\n".join(level_parts)

    prompt = MERGE_EVALUATION_PROMPT.format(
        goal=goal, steps_text=steps_text, level_steps_text=level_steps_text,
    )
    ...
    try:
        ...
        if not merges:
            logger.info("  LLM merge evaluation: no merges recommended. %s", reasoning)
            return steps
        ...
        for pair in merges:
            if (
                len(pair) == 2
                and all(isinstance(x, int) and 1 <= x <= len(steps) for x in pair)
                and pair[0] != pair[1]
                and pair[0] not in used_ids
                and pair[1] not in used_ids
                and level_set.get(pair[0]) == level_set.get(pair[1])
            ):
                valid_merges.append(pair)
                used_ids.add(pair[0])
                used_ids.add(pair[1])

        if not valid_merges:
            logger.warning("  LLM merge evaluation returned no valid pairs, skipping merge.")
            return steps
        ...
        for sid_a, sid_b in valid_merges:
            a = steps[sid_a - 1]
            b = steps[sid_b - 1]
            combined_title = f"{a['title']} + {b['title']}"
            combined_desc = (
            ...
            combined_deps = sorted(set(
                a.get("depends_on", []) + b.get("depends_on", [])
            ))
            ...
            merged_ids.add(sid_a - 1)
            merged_ids.add(sid_b - 1)
        ...
        logger.info(
            ...
            len(steps), len(merged_steps), reasoning,
        )
        return merged_steps
